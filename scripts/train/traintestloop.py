...
def train_loop_spectformer(dataloader, model, loss_fn, optimizer=None, lr_scheduler=None):
    ...
    for batch, data in enumerate(dataloader):
        ...
        # predictions and true labels 
        pred_arr = np.append(pred_arr, np.concatenate( (predictions.unsqueeze(1).cpu().detach().numpy(), target.unsqueeze(1).cpu().detach().numpy()), axis=1), axis=0)
...
def test_loop_spectformer(dataloader, model, loss_fn):
    # Set the model to evaluation mode - important for batch normalization and dropout layers
    # Unnecessary in this situation but added for best practices
    device = next(model.parameters()).device
    model.eval()
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            ...
            predictions = torch.where(pred >= 0.5, 1, 0)
            target = data['label'].squeeze(dim=1)
            test_loss += loss_fn(pred, target).item() #torch.nn.functional.log_softmax(pred, dim=1)
            pred_arr = np.append(pred_arr, np.concatenate( (predictions.unsqueeze(1).cpu().detach().numpy(), target.unsqueeze(1).cpu().detach().numpy()), axis=1), axis=0)
